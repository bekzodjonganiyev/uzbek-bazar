...
export const ProductCard = (props: Props): ReactElement => {
  ...
  const createMutation = useMutation({
    mutationFn: (variables: { url: string; data: any }) =>
      http.post(variables.url, variables.data),
    onSuccess: (data) => {
      ...
      toast({
        description:
          url === "carts"
            ...
            : "Maxsulot sevimlilarga qo'shildi",
        variant: "success",
        action: (
          <ToastAction
            ...
            altText="Try again"
            onClick={() =>
              navigate(`${url === "carts" ? "/cart" : "/favourites"}`)
            }
          >
  ...
  const deleteMutation = useMutation({
    mutationFn: (variables: { url: string; data: any }) =>
      http.delete(variables.url, variables.data),
    onSuccess: (data) => {
      const url = data.config.url?.split("/")[0];
      toast({
        description:
          url === "carts"
            ? "Maxsulot savatdan o'chirildi"
            : "Maxsulot sevimlilardan o'chirildi",
        variant: "danger",
      });
    },
    onError: (err) => console.log(err),
  ...
  if (props.row) {
    return (
      ...
      <div
        className={cn(
          "sm:p-3 p-0 flex items-start min-[365px]:justify-between sm:gap-1 gap-0 border"
        )}
      >
        ...
        <div className="md:w-4/12 sm:w-6/12 w-4/12 max-[365px]:mr-9">
          <h3
            className={cn("leading-none line-clamp-2 sm:text-base text-[14px]")}
          >
            {props.productName}
          </h3>
          {props.rating ? (
          ...
          <div className="flex flex-col">
            <div className="h-[12px]">
              <b className="text-xs">Material:</b>
              <span className="ml-1 text-xs">{props.material}</span>
            </div>
            <div className="h-[12px]">
              <b className="text-xs">Minimum sold:</b>
              <span className="ml-1 text-xs">{props.minimumSold}</span>
            </div>
            <div className="h-[12px]">
              <b className="text-xs">Season:</b>
              <span className="ml-1 text-xs">{props.season}</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col justify-between items-end gap-5">
            ...
            <div className="flex">
              ...
              >
                <LikeIcon
                  color={`${
                    productIdsForWishlist?.includes(props.id)
                      ? "#121212"
                      : "white"
                  }`}
                />
              ...
              >
                <CartIcon
                  ...
                  height={25}
                  color={`${
                    productIdsForCart.includes(props.id) ? "white" : "#121212"
                  }`}
                  type={`${
                    productIdsForCart.includes(props.id) ? "in" : "add"
                  }`}
                />
              </button>
            </div>
  ...
  return (
    ...
    <>
      <div className={cn("group relative rounded-md mb-3 ")}>
        ...
        {props.newBadge ? (
          <span className="absolute font-medium px-3 py-1 bg-white rounded left-2 top-2 md:text-sm text-xs">
            NEW
          </span>
        ) : null}
        ...
        {props.discount ? (
          <span
            className={`absolute font-medium px-3 py-1 bg-green-400 text-white rounded left-2 md:text-sm text-xs ${
              props.newBadge ? "top-12" : "top-2"
            }`}
          >
            {props.discount}
          </span>
        ...
        <div
          className={`${
            props.rating ? "text-left" : "text-center"
          } py-2 font-semibold`}
        >
          <div className="flex items-end justify-between">
            <div className="max-md:w-9/12 w-full ">
              ...
              <div
                className={`${props.rating ? "" : "justify-center"} flex gap-2`}
              >
                <p className="md:text-sm text-xs">{props.price}$</p>
                {props.oldPrice ? (
                  <del className="text-stone-300 font-thin md:text-sm text-xs">
                    {props.oldPrice}$
                  </del>
                ) : null}
              </div>
